...
function AdminDashboard() {
    ...
    console.log("🚀 ~ file: AdminDashboard.tsx:102 ~ AdminDashboard ~ data:", data)


    useEffect(() => {
        const fetchingData = async () => {
            const users = await fetchData('/user/listUsers')
            const theatres = await fetchData('/theatre/allTheatreDetails')
            const theatreMovies = await fetchData('/movie/listTheatreMovies')
            const ottMovies = await fetchData('/movie/listOttMovies')
            const payments = await fetchData('/payment/getAllPayments')
            console.log("🚀 ~ file: AdminDashboard.tsx:112 ~ fetchingData ~ payments:", payments)

            setUsers(users.data.users)
            setTheatres(theatres.data)
            setTheatreMovies(theatreMovies.data)
            setOttMovies(ottMovies.data)
    ...
    return (
        <>
            <div className="p-4 pt-16">
                ...
                <div className='grid  md:grid-cols-4 gap-3 grid-cols-2 py-5'>
                    <div className="bg-gray-800 p-5 rounded flex flex-col">
                        <div className="flex gap-10">
                            <h1 className="text-lg font-bold text-white">Total Users</h1>
                        ...
                        <div>
                            {
                                users && (
                                    <>
                                        <div>
                                            <h1 className="text-white text-4xl font-bold pl-4">{users.length}</h1>
                                        </div>
                                    </>
                                )
                            }
                    ...
                    <div className="bg-gray-800 p-5 rounded flex flex-col">
                        <div className="flex gap-10">
                            <h1 className="text-lg font-bold text-white">Total Theatres</h1>
                            <GiTheaterCurtains className="text-orange-500 text-4xl" />
                        </div>
                        <div>
                            {
                                theatres && (
                                    <>
                                        <div>
                                            <h1 className="text-white text-4xl font-bold pl-4">{theatres.length}</h1>
                                        </div>
                                    </>
                                )
                            }
                    ...
                    <div className="bg-gray-800 p-5 rounded flex flex-col">
                        <div className="flex gap-10">
                            <h1 className="text-lg font-bold text-white">Total Theatre Movies</h1>
                            <MdMovieCreation className="text-yellow-500 text-4xl" />
                        </div>
                        <div>
                            {
                                theatreMovies && (
                                    <>
                                        <div>
                                            <h1 className="text-white text-4xl font-bold pl-4">{theatreMovies.length}</h1>
                                        </div>
                                    </>
                                )
                            }
                    ...
                    <div className="bg-gray-800 p-5 rounded flex flex-col">
                        <div className="flex gap-10">
                            <h1 className="text-lg font-bold text-white">Total Ott Movies</h1>
                            <FaLaptop className="text-purple-500 text-4xl" />
                        </div>
                        <div>
                            {
                                ottMovies && (
                                    <>
                                        <div>
                                            <h1 className="text-white text-4xl font-bold pl-4">{ottMovies.length}</h1>
                                        </div>
                                    </>
                                )
                            }
