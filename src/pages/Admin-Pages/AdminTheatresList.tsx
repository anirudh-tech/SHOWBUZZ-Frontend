          ...
          <>
            ...
            <div className='flex justify-end pe-5'>
              <Badge badgeContent={approvals?.length} color="primary">
                <button onClick={() => setApprovalModal(true)} className='border border-1 border-red-500 text-white hover:bg-red-500 p-3 rounded'>Approval Requests</button>
              </Badge>
            </div>
                          ...
                          <TableCell key={subIndex}>
                            ...
                            {key === 'status' && ( // Check if key is 'status'
                              value === 'active' ? ( // If status is 'active', render block button
                                <button className='bg-gradient-to-r from-red-200 via-red-500 to-red-800 hover:from-red-800 hover:via-red-500 hover:to-red-200 rounded-md px-6 text-white p-3' onClick={() => toggleUserStatus(row._id, value)}>Block</button>
                              ) : ( // If status is 'blocked', render activate button
                                <button className='bg-gradient-to-r from-green-200 via-green-500 to-green-800 hover:from-green-800 hover:via-green-500 hover:to-green-200 rounded-md px-6 text-white p-3' onClick={() => toggleUserStatus(row._id, value)}>Activate</button>
                              )
                            )}
                          </TableCell>
                        ))}
