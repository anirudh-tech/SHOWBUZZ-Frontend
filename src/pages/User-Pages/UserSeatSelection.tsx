...
const UserSeatSelection = () => {
  ...
  console.log("🚀 ~ file: UserSeatSelection.tsx:20 ~ UserSeatSelection ~ time:", state)
  useEffect(() => {
  ...
  return (
    <>
      ...
      <div className='flex justify-center'>
        <div className='shadow-lg  shadow-orange-300 text-center cursor-pointer bg-white rounded-md w-1/3 h-24 py-3 flex flex-col justify-center items-center'>
          <h1 className="text-black font-roboto text-2xl font-bold">Screen Here</h1>
          <h1 className="text-black ">{numberOfSeats}</h1>
        </div>
      </div>
          ...
          <div key={index1} className="flex gap-5 justify-center">
            {
              row.map((column: any, index2 : number) => (
                <div onClick={() => handleSelectSeat(column.number)} className={` flex justify-center items-center rounded-t-md p-2 w-8 text-xs ${column.booked ? 'bg-white/20 text-white' : 'border shadow-md  shadow-green-300'} ${selectedSeats.includes(column.number) ? 'bg-green-700 text-white' : 'bg-white text-black'}`} key={index2}>
                  <button className="font-roboto font-semibold text-center">{column.number}</button>
                </div>
              ))
            }
          </div>
